...
func (c *Controller) HandleSignupPost() http.HandlerFunc {
	v := view.View{
		MainTmpl: "signedup.tmpl",
	...
	return func(w http.ResponseWriter, r *http.Request) {
		r.ParseForm()
		formParams := r.Form
		feedback := validate(formParams)

		//If they are then post them off to the API
		if len(feedback) == 0 {
			sm := models.NewSignUpModel(c.Session)
			...
			if !created {
				...
				feedback = append(feedback, "Head over to "+c.Config.PageContext.FullURL+"/myradio to get started.")
...
func validate(formParams url.Values) (feedback []string) {
